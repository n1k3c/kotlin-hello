...
fun albumAndTrackLowerThanXSeconds(durationInSeconds: Int, albums: List<Album>): List<Pair<String, String>> {
    // [a, b, c] f(x) => [f(a), f(b), f(c)]   ----> map
    // [[a, b], [c, d] f(x) => [f(a), f(b), f(c), f(d)]   ----> flatmap
...
fun main(args: Array<String>) {
    ...
    albums.forEach {
        it -> if (it.chartUK == 1) println("Found it - ${it.title}")
    }

    val albums1 = albums

    albums1.filter { it.chartUK == 1 }.forEach {
    ...
    val titles = albums1.filter { it.chartUK == 1 }
